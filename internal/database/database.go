...
func InitDB(cfg config.FlowEventsConnectorConfig, dbVersion uint) (*Postgres, error) {
	ctx, cancelFunc := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancelFunc()

	db := newPostgres(*cfg.Datastore)
	err := db.Connect(ctx)
	if err != nil {
		return nil, err
	}
...
func newPostgres(config config.DatastoreConfig) *Postgres {
	return &Postgres{
